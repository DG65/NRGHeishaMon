...
- Extra-Datenblock der HeishaMon-Firmware unterstützt (Anlass: Firmware-Update auf 4.2.2, dessen Doku die Sache klarstellt). Wärmepumpen der K/L-Serie und neuer senden einen zusätzlichen Datenblock, den die Platine als `extra/Heat_Power_Consumption_Extra`, `…Cool_…`, `…DHW_…` (Aufnahme) und `…Heat_Power_Production_Extra`, `…Cool_…`, `…DHW_…` (Wärmeleistung) veröffentlicht (seit Firmware 4.1.7). Bei solchen Anlagen liefern die alten `main/*_Power_*`-Topics laut Firmware-Doku ungültige Werte (z. B. -200). Das Modul kannte `extra/` bisher nicht und bildete daraus die elektrische Leistung (ohne externen Zähler), die thermische Gesamtleistung und die COP-Schätzung - auf solchen Anlagen waren diese Werte falsch. Neu: sechs Datenpunkte (Gruppe Leistung & COP, Idents `Heat_Power_Consumption_Extra` usw.), und sobald mindestens ein Extra-Wert der jeweiligen Art vorliegt, zählen für die Summen ausschließlich die Extra-Werte; ohne Extra-Datenblock ändert sich nichts. Kein Vertragsbruch: `PowerID` und die übrigen Felder bleiben, nur die Werte dahinter stimmen jetzt. Einschränkung: wer die Extra-Datenpunkte in der Liste abwählt, bekommt sie nicht angelegt und damit weiter die alten Werte
- Doku-Fehler korrigiert: der Integrations-Abschnitt verwies auf einen GitHub-Link "im Forum-Hinweis", der dort seit 1.28.0 nicht mehr steht

## 1.30.0 — 2026-09-19

- Vertrag erweitert (contractVersion 1.13 → 1.14, additiv) für die Panasonic-Bedienung im NRGDashboard-WP-Monitor. `HEISHA_GetHeatingCurve()` liefert je Zone zusätzlich `shift` (Heizen/Kühlen: `mode` `'shift'` für Verschiebung -5..+5, `'direct'` für die direkte Vorlauf-Solltemperatur, sonst `null`, dazu `valueC`) und oben `boardRulesActive`; neu `HEISHA_SetHeatingCurveShift()`, das nur im Verschiebungsmodus sendet (die Firmware klemmt nichts, im Direktmodus käme ein "+2" als 2 °C Vorlauf an). Neu `HEISHA_GetOperations()`/`HEISHA_SetOperation()` für Flüsterbetrieb, Leistungsbetrieb, Urlaub (nur an/aus, kein Zeitraum möglich), Notbetrieb (Heizstab) und Warmwasser-Soll (40..75 °C). Alle Sets liefern `true` nur für „abgeschickt“, schreiben nicht optimistisch in die Variablen und ergeben `false` bei ungültiger Eingabe/fehlendem Gateway. Am Firmware-Quelltext geprüft, mit Dashboard abgestimmt, bei EMS erfasst. Bekannte Wechselwirkung: das optionale Taktschutz-Regelwerk arbeitet mit der Verschiebung 0/-5 und kann eine gesetzte Verschiebung überschreiben bzw. wird bei Verschiebung ungleich 0 wirkungslos - deshalb `boardRulesActive`
